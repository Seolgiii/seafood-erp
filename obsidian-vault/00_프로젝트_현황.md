...
■ 최근 변경 (2026-05-07)
- 옵시디언 vault 시나리오 **23개** 발굴(A7+B4+C3+D4+E2+F3) + 모듈/결정/시나리오 양방향 [[link]] (모듈→시나리오 102 / 결정→시나리오 84 / 시나리오↔시나리오 64 / 시나리오→모듈 233 / 시나리오→결정 108)
- QR 통일안 결정 — LOT 평생 식별자(입고 시 생성), 스캔 시 통합 LOT 랜딩 페이지(/lot/{번호})로 라우팅. `QR_LOT_식별자_통합` 신규 + 기존 2개 정합 (구현 시점 미정)
- `/wrap-up` 명령에 옵시디언 동기화 단계 4.5 추가 (4.5-A: CLAUDE.md→00 / 4.5-B: z.Mission→01 / 4.5-C: journal→10_작업일지/{날짜}) + 격리 안전장치
